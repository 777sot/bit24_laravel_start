...
class ValuesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($data as $k => $value) {
            ...
            $field = Field::where('member_id', $member_id)->where('CRM_TYPE', 'CRM_QUOTE')->find($k);
            if ($field) {
        ...
        $new_values = Value::where('member_id', $member_id)->where('CRM_TYPE', 'CRM_QUOTE')->where('ENTITY_VALUE_ID', $ENTITY_VALUE_ID)->get();
        $values_data = [];
        $results = [];
        ...
        $res = MyB24::CallB24_upd_values('CRM_QUOTE', $update);
    ...
    public function show(Request $request, string $id)
    {
        ...
        $values = Value::where("CRM_TYPE", "CRM_QUOTE")
        ...
        $fields = Field::where("CRM_TYPE", "CRM_QUOTE")
            ->where("member_id", $data['member_id'])
            ->get();

        foreach ($fields as $field) {

            $val = Value::where("CRM_TYPE", "CRM_QUOTE")
                ->where("member_id", $data['member_id'])
                ->where("ENTITY_VALUE_ID", $id)
