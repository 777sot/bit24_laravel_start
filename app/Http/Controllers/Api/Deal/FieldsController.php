...
use App\Http\Requests\Api\Deal\Fields\StoreRequest;
use App\Http\Requests\Api\Deal\Fields\UpdateRequest;
use App\Http\Requests\Api\Deal\Fields\ShowRequest;
use App\Http\Resources\Api\Deal\FieldResource;
...
class FieldsController extends Controller
{
    ...
    public function index(ShowRequest $request)
    {
        ...
        $fields = Field::where('CRM_TYPE', 'CRM_DEAL')->where('member_id', $data['member_id'])->get();
        ...
        if ($field) {
            ...
            if ($field->USER_TYPE_ID == 'enumeration') {
                ...
                if (empty($field->BTX_ID)) {
                    $add_field_list = MyB24::CallB24_field_enumeration_add_new("CRM_DEAL", $data);
                    if (isset($add_field_list['result'])) {
                        $btx_id = $add_field_list['result'];
                        $field->BTX_ID = $btx_id;
                        $field->save();
                        //ДОБАВЛЕНИЕ LIST BTX_ID
                        $usr_field_list = MyB24::CallB24_field_list_new("CRM_DEAL", $field->member_id, $field->BTX_ID);
                ...
                if (empty($field->BTX_ID)) {
                    // string
                    $add_field_text = MyB24::CallB24_field_text_add_new("CRM_DEAL", $data);
    ...
    public function update(UpdateRequest $request, string $id)
    {
        ...
        if ($field) {
            ...
            if ($field->USER_TYPE_ID == 'enumeration' && !empty($field->BTX_ID)) {
                ...
                $add_field_list = MyB24::CallB24_field_enumeration_upd_new("CRM_DEAL", $data, $field->BTX_ID);
                if (isset($add_field_list['result'])) {
                    $btx_id = $add_field_list['result'];
                    $field->BTX_ID = $btx_id;
                }
            ...
            if ($field->USER_TYPE_ID == 'string' && !empty($field->BTX_ID)) {
                ...
                $upd_field_text = MyB24::CallB24_field_text_upd_new("CRM_DEAL", $data);
            }
            ///---
    ...
    public function destroy(ShowRequest $request, string $id)
    {
        ...
        if ($field) {
            ...
            if (!empty($field->BTX_ID)) {
                ...
                $del_field = MyB24::CallB24_field_del_new("CRM_DEAL", $field->member_id, $field->BTX_ID);
                ///---
            }
    ...
    public function update_index(Request $request)
    {
        ...
        $count_fields = Field::where('CRM_TYPE', "CRM_DEAL")->count();
